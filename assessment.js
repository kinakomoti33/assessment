...
assessmentButton.addEventListener(
  'click',
  function(){
    const userName = userNameInput.value;//入力内容を取得
    if(userName.length ===0){//入力が空だったら
      //名前が空の時は処理を終了する
      return;
    }
//診断結果表示エリアの作成
    resultDivision.innerText='';//divタグをから文字列で上書きで空にしている。
    const header = document.createElement('h3');//h3タグの作成
    header.innerText = '診断結果';
    resultDivision.appendChild(header);//divタグの子要素として追加

    const paragragh = document.createElement('p')//タグの作成
...
function assessment (userName){
  ...
  //合計値を配列の要素数であまりをとることで、配列の要素数で抑えることができる。
